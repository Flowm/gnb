...
-- Tue 26 Jan 2016 03:01:49 PM CET
		...
		1000000000,		# balance is 1 billion
			SUM(		# else calculate sum
				IF (
					...
					0,														# then do not use for balance
					IF (													# otherwise check
						A.id = T.destination_account_id,					# if we are destination
						T.amount,											# then count as plus
						-T.amount											# otherwise count as minus
					)
				)
			)
	) AS balance
