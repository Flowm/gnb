...
\vultable{\doge}{%
	...
	Vulnerabilities with creating the client account have been discussed in the previous section \vulnref{OTG-IDENT-002}, but using Bug No.2 an non-authenticated user is able to approve a client user request.
...
\vultable{\doge}{%
	Through direct page request, parameter modification, and SQL injection a non-authenticated user is able to gain access to both client and employee functions see \vulnref{OTG-AUTHZ-002} and \vulnref{OTG-AUTHZ-004} for more details on how to exploit such bugs, from more details on what is possible, please refer to \ref{figure:RoleDefinitionsDoge}. %TODO: Split first sentence (direct page request and SQL injection, bug is only for direct page request)
