...
\vultable{\doge}{%
	The batch transaction functionality at \url{http:/HOST/tran.php} does not seem to check the extension of the uploaded files.\newline
...
\vultable{\gnb}{%
	...
	An attacker could easily brute-force the names of files and folders cointained inside the root folder of the web application (perhaps using an appropriate tool). Figuring out the existence of such vulnerability is, hence, doomed to happen.
}{%
...
\vultable{\doge}{%
	...
	Vulnerabilities with creating the client account have been discussed in the previous section \vulnref{OTG-IDENT-002}, but using  \bugref{2} an non-authenticated user is able to approve a client user request.
	
	Provisioning Employees is set up to only be possible by other employees, but using  \bugref{12}, a non-authenticated user is able to create an employee account.
}{%
...
\vultable{\doge}{%
	...
	For more details on what privileges can a non-authenticated user can exploite, please refer to \ref{figure:RoleDefinitionsDoge}.
	\begin{itemize}
...
\vultable{\gnb}{%
	...
	The same implications mentioned for the DogeBank case apply. However, since in this case an attacker could have gotten root access (refer to CONFIG 001), the implications are much worse.
}{%
\cvssBaseScorePretty{N}{L}{N}{N}{C}{L}{L}{L} %FIXME: This are mock values
}
%INPVAL 014
...
\vultable{\doge}{%
	...
	Finding out these vulnerabilities required multiple attempts. In order to find these vulnerabilities we performed manual attacks and used the Fuzz funcionality of the Zed Attack Proxy tool.
}{%
...
\vultable{\doge}{%
	We observed that the \doge{} is not validating user inputs. This includes the following fields from a business logic perspective:
	
	\begin{itemize}
	
	\item Transaction Destination Account number
...
While testing the DogeBank application, we found out several quite disturbing bugs and issues, which will be presented here. Although not all of them could be considered to be strictly related to Data Validation, they are still Business logic issues which we couldn't point out in other sections.
...
\vultable{\gnb}{%
	...
	\cvssBaseScorePretty{N}{L}{N}{N}{C}{L}{L}{L} %FIXME: These are mock values
}
