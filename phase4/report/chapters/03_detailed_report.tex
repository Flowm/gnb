...
\vultable{\bs}{%
	While analyzing the source code, we found out that the SessionID is generated entirely randomly, using the PHP APIs (this ca be seen inside the login callback function contained in \texttt{/api/index.php}). It is also used only for session tracking purposes, therefore no session puzzling vulnerability can occur.
}{%
...
\vultable{\bs}{%
	...
	\cvssBaseScorePretty{N}{L}{L}{N}{U}{H}{N}{H}
}
\vultable{\gnb}{%
...
\vultable{\gnb}{%
	...
	We statically analyzed the PHP source code using RIPS, RATS and later on went through the code manually, in order to find additional vulnerabilities not detected by the previously mentioned tools.
}{%
	\na
}{%
	\na
