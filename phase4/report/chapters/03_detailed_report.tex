...
\begin{itemize}
	...
	\item \textbf{Useful in}\\ \vulnref{OTG-CRYPST-001}.
...
\vultable{\bs}{%
	...
	\cvssBaseScorePretty{N}{L}{L}{N}{U}{L}{N}{L}
}
...
\vulntitle{OTG-INPVAL-014}{Testing for Buffer overflow}
	A Heap overflow was discovered and is described in
	\vulnref{OTG-INPVAL-014-3}.

\simpleVulntitle{OTG-INPVAL-014-2}{Testing for Stack overflow}
\vultable{\bs}{%
	No stack overflow was discoverd in the \bs{} application.
}{%
	In the process of reverse engineering the binary we were keeping track of
	all accesses to variables on the stack.
	Afterwards we inspected the resulting source code for potential stack
	overflows. This was done manually and with the RATS application.
}{%
	\na
}{%
	\na
...
\vultable{\gnb}{%
	No stack overflow was discoverd in the \gnb{} application.
}{%
	We inspected the source code for potential stack overflows. This was done
	manually and with the RATS application.
}{%
	\na
}{%
	\na
...
\vultable{\bs}{%
	We discoverd a heap overflow in the \bs{} application which can be
	triggered by invoking the application with arguments longer than 200
	characters. The details of this vulnerability are described in the reverse
	engineering chapter \ref{chapter:re_heap}.
}{%
	In the process of reverse engineering the binary we were keeping track of
	all accesses to the memory allocated on the heap.
	Afterwards we inspected the resulting source code for potential heap
	overflows. This was done manually and with the RATS application.
}{%
	Low. The attacker needs an user account on the machine as the relevant
	arguments for the parser can not be modified from the web interface.
}{%
	Low. The attacker already needs the permission to execute the programm with
	custom arguments so he gains litte additional benefits form exploiting this
	vulnerability.
}{%
	Change the strncpy call as described in \autoref{chapter:re_heap}
}{%
	\cvssBaseScorePretty{L}{L}{L}{N}{U}{N}{L}{N}
}
\vultable{\gnb}{%
	No stack overflow was discoverd in the \gnb{} application.
}{%
	We inspected the source code for potential heap overflows. This was done
	manually and with the RATS application.
}{%
	\na
}{%
...
\vultable{\bs}{%
	No format string issues were discoverd in the \bs{} application.
}{%
	In the process of reverse engineering the binary we were keeping track of
	all format string usages.
	Afterwards we inspected the resulting source code for potential issues with
	the format string. This was done manually and with the RATS application.
}{%
	\na
}{%
	\na
...
\vultable{\gnb}{%
	...
	We inspected the source code for potential issues with the format sting.
	This was done manually and with the RATS application.
}{%
	\na
}{%
	\na
...
\vultable{\bs}{%
	...
	\begin{itemize}
		...
		\item Disable SSLv3, Medium grade encryption, Triple DES etc.
	\end{itemize}
}{%
...
\vultable{\gnb}{%
	While testing the whole application, we discovered that only valid data is accepted, not allowing to exploit the application in any way by inserting invalid data. \newline
	As described in \autoref{chapter:reverse_engineering} (SCS analysis) however, after a specific point in time, user input will no longer be validated correctly, because of a timestamp overflow bug in the Java code. This cannot be considered a vulnerability, but will pose a problem for clients in the future (in 22 years time).
}{%
